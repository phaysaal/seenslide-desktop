...
# JWT secret key - load from environment or persist to file for stability across restarts
def _load_or_create_jwt_secret() -> str:
    """Load JWT secret from file, or create and persist a new one."""
    import os
    secret = os.environ.get("SEENSLIDE_JWT_SECRET")
    if secret:
        return secret
    secret_file = Path.home() / ".config" / "seenslide" / ".jwt_secret"
    try:
